...
if errorLevel 1 (
    ...
    reg delete "HKLM\TempDefaultProfile\Control Panel\Desktop" /v "ScreenSaveActive" /f >nul 2>&1
...
for /f "tokens=1,2 delims=\" %%a in ('reg query HKEY_USERS ^| findstr /r /c:"S-1-5-21-[0-9\-]*$"') do (
    reg delete "HKU\%%b\Control Panel\Desktop" /v "ScreenSaveActive" /f >nul 2>&1
